...
func (s *Stream) Write(data []byte) (n int, err error) {
	if err = s.conn.SetWriteDeadline(time.Now().Add(s.timeout)); err != nil {
		logger.Error("Failed to write message: %s", err)
		return
	}
...
func (s *Stream) ReadSome() (instruction []byte, err error) {
	if err = s.conn.SetReadDeadline(time.Now().Add(s.timeout)); err != nil {
		logger.Error("failed to set deadline: %s", err)
		return
	}
...
// See https://guacamole.apache.org/doc/gug/protocol-reference.html
// for a protocol reference of guacd
func (s *Stream) Handshake(config *Config) error {
	...
	for i, argName := range argNameS {
		...
		if len(value) == 0 {
			...
			if i == 0 {
				// The first argument should be the client version
				value = "VERSION_1_5_0"
			}
		}
		...
		// Log value
		logger.Trc("Received argument %d from guacamole: %s -> %s", i, argName, value)
	}
